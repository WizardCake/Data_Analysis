---
title: "Análise da Geração, Consumo e Consumidores"
subtitle: 'de Energia Elétrica no Brasil'
date: "`r Sys.Date()`"
...
```{r echo=FALSE}
pkgs <- cite_packages(output = "table", out.dir = ".", include.RStudio = T)
knitr::kable(pkgs)
```
...
```{r fig.height=8, fig.width=10}
# Pipeline para processar a média dos dados estado x classe e plotar
consumo %>%
  dplyr::summarise(Consumo = sum(Consumo),
  ...
  dplyr::mutate(Total = sum(Comercial, Industrial, Outros, Residencial, Rural,
                            na.rm = TRUE),
                across(c(Comercial:Rural), ~ .x / Total)) %>%
  dplyr::ungroup() %>%
  tidyr::pivot_longer(cols = c(Comercial:Rural)) %>%
  dplyr::right_join(estados, by = c('UF' = 'abbrev_state')) %>%
  ...
  dplyr::mutate(mean_value = mean(value,
                ...
                .by = c(DataExcel, name_region, name)) %>% 
  dplyr::select(DataExcel, UF, name, name_region, value, mean_value) %>% 
  ggplot() +
  aes(x = DataExcel) +
